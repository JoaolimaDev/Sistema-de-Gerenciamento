...
public class FileNodeServiceImpl implements FileNodeService {
    ...
    public FileNode updateFileNode(FileNodePutRequest fileNodePutRequest, String name) {
        ...
        if (!fileNodePutRequest.newName().equals(fileNode.getName())) {
            ...
            if (existingNodeOptional.isPresent()) {
                throw new CustomException("O nome fornecido já está reservado: " + existingNodeOptional.get().getName(), HttpStatus.BAD_REQUEST);
            }
        }
